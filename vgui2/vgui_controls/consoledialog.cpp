...
CHistoryItem::CHistoryItem( const char *text, const char *extra )
{
	...
	m_text = text;
	m_extraText = extra;
	m_bHasExtra = !m_extraText.IsEmpty();
}
...
CHistoryItem::CHistoryItem( const CHistoryItem& src )
{
	...
	m_extraText = NULL;
	m_bHasExtra = false;
	SetText( src.m_text, src.m_extraText );
}
...
{
	return m_text.IsEmpty() ? NULL : m_text.Get();
}

const char *CHistoryItem::GetExtra() const
{
	return m_extraText.IsEmpty() ? NULL : m_extraText.Get();
}

void CHistoryItem::SetText( const CUtlString& text, const CUtlString& extra )
{
	m_text = text;
	m_extraText = extra;
	m_bHasExtra = !m_extraText.IsEmpty();
...
void CConsolePanel::AddToHistory( const CUtlString &command, const CUtlString &extra )
{
