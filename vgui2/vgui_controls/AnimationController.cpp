...
// Purpose: Runs a custom command from code, not from a script file
//-----------------------------------------------------------------------------
void AnimationController::RunAnimationCommand( vgui::Panel *panel, const char *variable, Value_t targetValue, float startDelaySeconds, float duration, Interpolators_e interpolator, float animParameter /* = 0 */ )
{
	// clear any previous animations of this variable
	UtlSymId_t var = g_ScriptSymbols.AddString( variable );
	RemoveQueuedAnimationByType( panel, var, UTL_INVAL_SYMBOL );
...
template < class T >
void AnimationController::RunAnimationCommand( T* panel, const char* variable, float targetValue, float startDelaySeconds, float duration, Interpolators_e interpolator, void( T::* callback )( ), float animParameter )
{
	// clear any previous animations of this variable
	UtlSymId_t var = g_ScriptSymbols.AddString( variable );
	RemoveQueuedAnimationByType( panel, var, UTL_INVAL_SYMBOL );
	...
	animateCmd.startTime = startDelaySeconds;
	animateCmd.duration = duration;
	animateCmd.callback = CreateFunctor( panel, callback );
