...
void BuildGroup::RegisterControlSettingsFile(const char *controlResourceName, const char *pathID)
{
	...
	if ( sym.String()[0] && !m_RegisteredControlSettingsFiles.IsValidIndex(m_RegisteredControlSettingsFiles.Find(sym)))
	{
...
void BuildGroup::ReloadControlSettings()
{
	...
	if (!m_pResourceName.IsEmpty())
	{
		...
		if (edit)
		{
			edit->LoadControlSettings(m_pResourceName, m_pResourcePathID.IsEmpty() ? NULL : m_pResourcePathID.Get() );
		}
		else
		{
			LoadControlSettings(m_pResourceName, m_pResourcePathID.IsEmpty() ? NULL : m_pResourcePathID.Get() );
		}
	}
...
void BuildGroup::ChangeControlSettingsFile(const char *controlResourceName)
{
	...
	if (edit)
	{
		edit->LoadControlSettings(controlResourceName, m_pResourcePathID.IsEmpty() ? NULL : m_pResourcePathID.Get() );
	}
	else
	{
		LoadControlSettings(controlResourceName, m_pResourcePathID.IsEmpty() ? NULL : m_pResourcePathID.Get() );
	}
...
bool BuildGroup::SaveControlSettings( void )
{
	...
	if ( !m_pResourceName.IsEmpty() )
	{
		...
		char fullpath[ 512 ];
		g_pFullFileSystem->RelativePathToFullPath( m_pResourceName, m_pResourcePathID.IsEmpty() ? NULL : m_pResourcePathID.Get(), fullpath, sizeof( fullpath ) );
