...
public:
	...
	void AddItem( Panel *label, TextEntry *edit, ComboBox *combo, Button *button, const char *name, PanelSettingType_t type )
	{
		PanelItem_t item;
...
{
	static CFmtStr closeBuildDialogsName( "build_dialog_close_%s", GetControlsModuleName() );
	static ConCommand closeBuildDialogs( closeBuildDialogsName, [] { g_BuildModeDialogMgr.CloseAll(); } );
...
void CBuildModeDialogMgr::CloseAll()
{
	const int count = m_vecBuildDialogs.Count();
	for ( int i = count - 1; i >= 0; --i )
	{
		m_vecBuildDialogs[i]->Close();
	}
}
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
BuildModeDialog::BuildModeDialog(BuildGroup *buildGroup) : Frame(/*buildGroup->GetContextPanel()*/ NULL, "BuildModeDialog")
{
...
void BuildModeDialog::CreateControls()
{
	...
	m_pShowHelp = new Button( this, "HelpButton", "&Help", this, "ShowHelp" );
	m_pShowHelp->SetSize( 64, buttonH );

	m_pToggleRulers = new Button( this, "ToggleRulersButton", "&Toggle rulers", this, "toggle" );
	m_pToggleRulers->SetSize( 84, buttonH );

	m_pReset = new Button( this, "ResetButton", "&Reset", this, "RevertToSaved" );
	m_pReset->SetSize( 64, buttonH );
	...
	m_pToggleRulers->SetTabPosition( 12 );
	m_pShowHelp->SetTabPosition( 13 );

	m_pReloadLocalization->SetTabPosition( 14 );
	m_pReset->SetTabPosition( 15 );
}
...
void BuildModeDialog::ApplySchemeSettings( IScheme *pScheme )
{
	...
	m_pShowHelp->SetFont( font );
	m_pToggleRulers->SetFont( font );
	m_pReset->SetFont( font );
}
...
void BuildModeDialog::PerformLayout()
{
	...
	xpos -= m_pReset->GetWide();
	xpos -= XGAP;
	m_pReset->SetPos( xpos, ypos );
	...
	m_pShowHelp->SetPos( xpos, ypos - m_pVarsButton->GetTall() - 2 );
	m_pToggleRulers->SetPos( xpos + m_pShowHelp->GetWide() + 4, ypos - m_pVarsButton->GetTall() - 2 );
...
void BuildModeDialog::SetActiveControl(Panel *controlToEdit)
{
	...
	const size_t numSettings = m_pCurrentPanel->GetNumPanelSettings();

	// parse out the control description
	int tabPosition = 1;
	for ( size_t i = 0; i < numSettings; i++ )
	{
		const PanelSetting_t& setting = m_pCurrentPanel->GetSetting( i );

		// get the field name
		const char *fieldName = setting.name;
		const PanelSettingType_t datat = setting.type;
...
void BuildModeDialog::OnCommand(const char *command)
{
	...
	else if (!stricmp(command, "toggle"))
	{
...
void BuildModeDialog::ApplyDataToControls()
{
	...
	// create a section to store settingsKeyValues *dat = nullptr;
