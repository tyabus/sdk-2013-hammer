...
//-----------------------------------------------------------------------------
DECLARE_ATTRIBUTE_TYPE( CDmxElement*,		AT_ELEMENT,				"element",		value = nullptr; )
...
public:
	...
	template <typename T> const CUtlVector<T>& GetArray() const;
	...
	template <typename T> CUtlVector<T>& GetArrayForEdit();
...
private:
	...
	CDmxAttribute( const CUtlSymbol& attributeName );
	~CDmxAttribute();
...
void CDmxAttribute::SetValue( const T& value )
{
	...
	CopyConstruct( static_cast<T*>( m_pData ), value );
}
...
{
	if ( m_Type == AT_STRING )
		return *static_cast<CUtlString*>( m_pData );
	return "";
}
...
inline const CUtlVector<T>& CDmxAttribute::GetArray() const
{
	if ( CDmAttributeInfo<CUtlVector<T>>::AttributeType() == m_Type )
		return *static_cast<CUtlVector<T>*>( m_pData );
...
inline CUtlVector<T>& CDmxAttribute::GetArrayForEdit()
{
	...
	Construct( static_cast<CUtlVector<T>*>( m_pData ) );
	return *static_cast<CUtlVector<T>*>( m_pData );
}
