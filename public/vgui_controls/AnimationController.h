...
public:
	...
	void RunAnimationCommand( vgui::Panel *panel, const char *variable, Value_t targetValue, float startDelaySeconds, float durationSeconds, Interpolators_e interpolator, float animParameter = 0 );
	template<class T> void RunAnimationCommand( T *panel, const char *variable, float targetValue, float startDelaySeconds, float duration, Interpolators_e interpolator, void ( T::*callback )( ), float animParameter = 0 );
