...
extern CUtlVector<CCoreDispInfo*> builderDisps;
void CVRadDLL::Release()
{
	if ( bHammerMode )
		builderDisps.PurgeAndDeleteElements();
}
...
void CVRadDLL::GetBSPInfo( CBSPInfo *pInfo )
{
	...
	pInfo->g_dispinfo = builderDisps.Base();
	pInfo->g_numdispinfo = builderDisps.Count();
