...
void Rasterizer::Build()
{
	...
	// Clamp to 0..1
	fMinX = Max<float>(0, fMinX);
	fMinY = Max<float>(0, fMinY);
...
private:
	...
	struct MeshData_t
	{
		...
		{
			m_VertexColors.Purge();
			m_VertexColors.AddVectorToTail( other.m_VertexColors );
			m_TexelsEncoded.EnsureCapacity( other.m_TexelsEncoded.Count() );
			V_memcpy( m_TexelsEncoded.Base(), other.m_TexelsEncoded.Base(), other.m_TexelsEncoded.Count() );
			m_nLod = other.m_nLod;
...
// ------------------------------------------------------------------------------------------------
inline float ComputeBarycentricDistanceToTri( const Vector& _barycentricCoord, Vector2D _v[3] )
{
...
static int GetTexelCount(unsigned int _resX, unsigned int _resY, bool _mipmaps)
{
	...
	{
		retVal += _resX * _resY;
		_resX = Max<int>(1, _resX >> 1);
		_resY = Max<int>(1, _resY >> 1);
	}
