...
int CNormalList::FindOrAddNormal( Vector const &vNormal )
{
	...
	for( int iDim=0; iDim < 3; iDim++ )
	{
		...
		gi[iDim] = Min<int>( gi[iDim], NUM_SUBDIVS );
...
bool BuildFacesamples( lightinfo_t *pLightInfo, facelight_t *pFaceLight )
{
	...
	CUtlVectorFixed<sample_t, SINGLE_BRUSH_MAP * 2> sampleData;
...
void CreateDirectLights ()
{
	...
	for (i=0; i< uiPatchCount; i++)
	{
		...
		{
			directlight_t* dl = AllocDLight( p->origin, true );
	...
	for (i=0 ; i<(unsigned)num_entities ; i++)
	{
		...
		else if (!strstr(name, "light_deferred"))
		{
...
void GatherSampleSkyLightSSE( SSE_sampleLightOutput_t &out, directlight_t *dl, int facenum,
							 ...
							 float flEpsilon )
{
	const bool bIgnoreNormals = ( nLFlags & GATHERLFLAGS_IGNORE_NORMALS ) != 0;
	const bool force_fast = ( nLFlags & GATHERLFLAGS_FORCE_FAST ) != 0;
							   ...
							   float flEpsilon )
{
	...
	for (int j = 0; j < nsky_samples; j++)
	{
		...
		const fltx4 validity = CmpGtSIMD( dots[0], ReplicateX4( EQUAL_EPSILON ) );
		...
		for ( int i = 1; i < normalCount; i++ )
		{
			...
			const fltx4 validity2 = CmpGtSIMD( dots[i], ReplicateX4 ( EQUAL_EPSILON ) );
								  ...
								  float flEpsilon )
{
	...
	delta -= pos;
	const fltx4 dist2 = delta.length2();
	...
	if ( bHasHardFalloff )
	{
		const fltx4 notPastFadeDist = CmpLeSIMD ( dist, ReplicateX4 ( dl->m_flEndFadeDistance ) );
	...
	const fltx4 falloffEvalDist = MinSIMD( dist, ReplicateX4( dl->m_flCapDist ) );
					   ...
					   float flEpsilon )
{
	...
	const fltx4 notZero = CmpGtSIMD( out.m_flDot[0], Four_Zeros );
...
void BuildFacelights (int iThread, int facenum)
{
	...
	for ( int grp = 0; grp < numGroups; ++grp )
	{
		...
		for ( int k = 0; k < 4; k++ )
		{
			v[k] = ( k < numSamples ) ? sample[k].pos : sample[numSamples - 1].pos;
			n[k] = ( k < numSamples ) ? sample[k].normal : sample[numSamples - 1].normal;
		}
		...
		if ( !l.isflat )
		{
			for ( int k = 0; k < numSamples; k++ )
				sample[k].normal = sampleInfo.m_PointNormals[0].Vec( k );
		}
...
void ConvertRGBExp32ToLinear(const ColorRGBExp32 *pSrc, Vector* pDst)
{
	...
	(*pDst)[0] = TexLightToLinear( pSrc->r, pSrc->exponent);
	(*pDst)[1] = TexLightToLinear( pSrc->g, pSrc->exponent);
	(*pDst)[2] = TexLightToLinear( pSrc->b, pSrc->exponent);
}
