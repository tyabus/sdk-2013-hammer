...
CDmElement* CImportKeyValueBase::CreateDmElement( const char* pElementType, const char* pElementName, DmObjectId_t* pId )
{
	...
	const DmElementHandle_t hElement = g_pDataModel->CreateElement( pElementType, pElementName, DMFILEID_INVALID, pId );
...
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::AddBoolAttribute( CDmElement* pElement, KeyValues* pKeyValues, const char* pKeyName, const bool* pDefault )
{
...
// Used to add typed attributes from keyvalues
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::AddIntAttribute( CDmElement* pElement, KeyValues* pKeyValues, const char* pKeyName, const int* pDefault )
{
	KeyValues* pKey = pKeyValues->FindKey( pKeyName );
	int nValue;
	if ( pKey )
...
bool CImportKeyValueBase::AddFloatAttribute( CDmElement* pElement, KeyValues* pKeyValues, const char* pKeyName, const float* pDefault )
{
	KeyValues* pKey = pKeyValues->FindKey( pKeyName );
	float flValue;
	if ( pKey )
...
// Used to add typed attributes from keyvalues
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::AddBoolAttributeFlags( CDmElement* pElement, KeyValues* pKeyValue, const char* pKeyName, int nFlags, const bool* pDefault )
{
...
bool CImportKeyValueBase::AddIntAttributeFlags( CDmElement* pElement, KeyValues* pKeyValue, const char* pKeyName, int nFlags, const int* pDefault )
{
...
bool CImportKeyValueBase::AddFloatAttributeFlags( CDmElement* pElement, KeyValues* pKeyValue, const char* pKeyName, int nFlags, const float* pDefault )
{
...
void CImportKeyValueBase::RecursivelyResolveElement( CDmElement* pElement )
{
	...
	while ( pAttribute )
	{
		...
		{
		case AT_ELEMENT:
			RecursivelyResolveElement( pAttribute->GetValueElement<CDmElement>() );
			break;
		...
		case AT_ELEMENT_ARRAY:
			{
				...
				for ( int i = 0; i < nCount; ++i )
					RecursivelyResolveElement( array[i] );
			}
			break;
		}
...
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::Unserialize( CUtlBuffer& buf, const char*, int, const char*, int, DmFileId_t fileid, DmConflictResolution_t, CDmElement** ppRoot )
{
	*ppRoot = nullptr;
	...
	const bool bOk = kv->LoadFromBuffer( "dmx file", buf );
	if ( bOk )
