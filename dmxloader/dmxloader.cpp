...
void* DMXAlloc( size_t size )
{
	...
	if ( !s_bInDMXContext )
		return nullptr;
	return s_DMXAllocator.Alloc( size, true );
}
...
{
	if ( !pElement )
		return buf.PutInt( ELEMENT_INDEX_NULL ); // invalid handle
...
bool CDmxSerializer::SaveElement( CUtlBuffer& buf, CDmxSerializationDictionary& list, CUtlRBTree<const char*>& stringTable, CDmxElement* pElement )
{
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		const unsigned short sym = stringTable.Find( pName );
		if ( sym == CUtlRBTree<const char*>::InvalidIndex() )
			return false;
...
{
	const unsigned short sym = stringTable.Find( pElement->GetTypeString() );
	if ( sym == CUtlRBTree<const char*>::InvalidIndex() )
		return false;
...
bool CDmxSerializer::Serialize( CUtlBuffer& buf, CDmxElement* pRoot, const char* pFileName )
{
	...
	for ( i = dict.FirstRootElement(); i != DMX_SERIALIZATION_HANDLE_INVALID; i = dict.NextRootElement(i) )
	{
		...
		const int nAttributes = pElement->AttributeCount();
	...
	const int nStrings = stringTable.Count();
	if ( nStrings > 65535 )
		return false;
...
{
	const int nElementIndex = buf.GetInt();
...
bool CDmxSerializer::UnserializeAttributes( CUtlBuffer& buf, CDmxElement* pElement, CUtlVector<CDmxElement*>& elementList, int nStrings, int* offsetTable, char* stringTable )
{
	...
	char nameBuf[1024];
	const int nAttributeCount = buf.GetInt();
	for ( int i = 0; i < nAttributeCount; ++i )
	{
		const char* pName;
		if ( stringTable )
		{
			const int si = buf.GetShort();
			if ( si >= nStrings )
				return false;
		...
		const DmAttributeType_t nAttributeType = static_cast<DmAttributeType_t>( buf.GetChar() );
...
int CDmxSerializer::GetStringOffsetTable( CUtlBuffer& buf, int* offsetTable, int nStrings )
{
	...
	const char* pBegin = static_cast<const char*>( buf.PeekGet() );
	const char* pBytes = pBegin;
	for ( int i = 0; i < nStrings; ++i )
	{
		...
		do
		{
			...
			{
				pBegin = static_cast<const char*>( buf.PeekGet( nBytes + 1, 0 ) );
				if ( !pBegin )
					return false;
...
bool CDmxSerializer::Unserialize( CUtlBuffer& buf, int nEncodingVersion, CDmxElement** ppRoot )
{
	...
	const bool bReadStringTable = nEncodingVersion >= 2;

	// Keep reading until we read a nullptr terminator
	while( buf.GetChar() != 0 )
	{
		if ( !buf.IsValid() )
			return false;
	...
	if ( bReadStringTable )
	{
		...
		offsetTable = static_cast<int*>( stackalloc( nStrings * sizeof( int ) ) );
		...
		stringTable = static_cast<char*>( stackalloc( nStringMemoryUsage * sizeof( char ) ) );
	...
	for ( int i = 0; i < nElementCount; ++i )
	{
		...
		CDmxElement* pElement = new CDmxElement( pType );
		{
			...
			pAttribute->SetValue( static_cast<char const*>( pName ) );
			pElement->SetId( id );
		}
...
bool SerializeDMX( CUtlBuffer& buf, CDmxElement* pRoot, const char* pFileName )
{
	...
	const int nEncodingVersion = buf.IsText() ? 1 : 2; // HACK - we should have some way of automatically updating this when the encoding version changes!
	const char* pFormatName = GENERIC_DMX_FORMAT;
	const int nFormatVersion = 1; // HACK - we should have some way of automatically updating this when the encoding version changes!
...
//-----------------------------------------------------------------------------
static bool ReadDMXHeader( CUtlBuffer& buf, char* pEncodingName, int nEncodingNameLen, int& nEncodingVersion, char* pFormatName, int nFormatNameLen, int& nFormatVersion )
{
	// Make the buffer capable of being read as text
	const bool bBufIsText = buf.IsText();
	const bool bBufHasCRLF = buf.ContainsCRLF();
...
bool UnserializeDMX( CUtlBuffer& buf, CDmxElement** ppRoot, const char* pFileName )
{
	...
	*ppRoot = nullptr;
	...
	if ( !ReadDMXHeader( buf, pEncodingName, sizeof( pEncodingName ), nEncodingVersion, pFormatName, sizeof( pFormatName ), nFormatVersion ) )
		return false;
	...
	const bool bIsBinary = ( buf.GetFlags() & CUtlBuffer::TEXT_BUFFER ) == 0;
	if ( bIsBinary )
	{
