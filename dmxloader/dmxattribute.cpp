...
// Attribute size
//-----------------------------------------------------------------------------
static constexpr const size_t s_pAttributeSize[AT_TYPE_COUNT] =
{
	0,							// AT_UNKNOWN,
...
CDmxAttribute::CDmxAttribute( const CUtlSymbol& attributeName )
{
...
void CDmxAttribute::FreeDataMemory()
{
	if ( m_Type != AT_UNKNOWN )
	{
		...
		if ( !IsArrayType( m_Type ) )
		{
			if ( m_Type == AT_STRING )
			{
				Destruct( static_cast<CUtlString*>( m_pData ) );
			}
			else if ( m_Type == AT_VOID	)
			{
				Destruct( static_cast<CUtlBinaryBlock*>( m_pData ) );
			}
		}
		else
		{
...
#define GET_ARRAY_COUNT( _dataType )																\
	case CDmAttributeInfo<CUtlVector<_dataType>>::ATTRIBUTE_TYPE:									\
	{																								\
		const CUtlVector<_dataType>& array = *static_cast<CUtlVector<_dataType>*>( m_pData );	\
		return array.Count();																		\
	}
...
#define SERIALIZE_TYPE( _buf, _attributeType, _dataType )						\
	...
	{																			\
		if ( m_pData )															\
			return ::Serialize( _buf, *static_cast<_dataType*>( m_pData ) );	\
		_dataType temp;															\
...
#define SERIALIZE_ARRAY_TYPE( _buf, _attributeType, _dataType )								\
	...
	{																						\
		if ( m_pData )																		\
			return ::Serialize( _buf, *static_cast<CUtlVector<_dataType>*>( m_pData ) );	\
...
#define UNSERIALIZE_TYPE( _buf, _attributeType, _dataType )					\
	case _attributeType:													\
		Construct( static_cast<_dataType*>( m_pData ) );					\
		return ::Unserialize( _buf, *static_cast<_dataType*>( m_pData ) )

#define UNSERIALIZE_ARRAY_TYPE( _buf, _attributeType, _dataType )			\
	case _attributeType:													\
		Construct( static_cast<CUtlVector<_dataType>*>( m_pData ) );		\
		return ::Unserialize( _buf, *static_cast<CUtlVector<_dataType>*>( m_pData ) )
...
bool CDmxAttribute::UnserializeElement( DmAttributeType_t type, CUtlBuffer& buf )
{
	...
	const bool bIsDataMemoryAllocated = ( m_Type == type );
	if ( !bIsDataMemoryAllocated )
	{
		AllocateDataMemory( type );
	}
...
void CDmxAttribute::SetValue( const char* pString )
{
	...
	CUtlString* pUtlString = static_cast<CUtlString*>( m_pData );
...
void CDmxAttribute::SetValue( const void* pBuffer, size_t nLen )
{
	...
	CUtlBinaryBlock* pBlob = static_cast<CUtlBinaryBlock*>( m_pData );
	Construct( pBlob );
...
{
	const DmAttributeType_t type = pAttribute->GetType();
	if ( !IsArrayType( type ) )
	{
...
void CDmxAttribute::SetValueFromString( const char* pValue )
{
	...
	default:
		{
			const int nLen = pValue ? Q_strlen( pValue ) : 0;
			if ( nLen == 0 )
			{
				SetToDefaultValue();
				break;
