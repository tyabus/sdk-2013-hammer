...
{
	const int nIndex = FindAttribute( pAttributeName );
	if ( nIndex >= 0 )
...
{
	CDmxElementModifyScope modify( this );
	const int idx = FindAttribute( pAttributeName );
	if ( idx >= 0 )
	{
...
{
	CDmxElementModifyScope modify( this );
	const int nCount = m_Attributes.Count();
...
// Find an attribute by name-based lookup
//-----------------------------------------------------------------------------
int CDmxElement::FindAttribute( const CUtlSymbol& attributeName ) const
{
	Resort();
...
void CDmxElement::AddElementsToDelete( CUtlVector<CDmxElement*>& elementsToDelete )
{
	...
	const int nCount = AttributeCount();
	for ( int i = 0; i < nCount; ++i )
	{
		...
		if ( pAttribute->GetType() == AT_ELEMENT_ARRAY )
		{
			...
			const int nElementCount = elements.Count();
...
void CDmxElement::RemoveAllElementsRecursive()
{
	...
	const int nCount = elementsToDelete.Count();
	for ( int i = 0; i < nCount; ++i )
...
//-----------------------------------------------------------------------------
void CDmxElement::UnpackIntoStructure( void* pData, size_t DataSizeInBytes, const DmxElementUnpackStructure_t* pUnpack ) const
{
	for ( ; pUnpack->m_AttributeType != AT_UNKNOWN; ++pUnpack )
	{
		char* pDest = static_cast<char*>( pData ) + pUnpack->m_nOffset;

		if ( pDest + pUnpack->m_nSize > static_cast<char*>( pData ) + DataSizeInBytes )
		{
			AssertMsg( 0, "CDmxElement::UnpackIntoStructure: Write structure overrun!\n" );
		...
		if ( !pAttribute )
		{
			...
			const int nLen = Q_strlen( pUnpack->m_pDefaultString ) + 1;
...
void CDmxElement::AddAttributesFromStructure_Internal( const void* pData, size_t byteCount, const DmxElementUnpackStructure_t* pUnpack )
{
	for ( ; pUnpack->m_AttributeType != AT_UNKNOWN; ++pUnpack )
	{
		const char* pSrc = static_cast<const char*>( pData ) + pUnpack->m_nOffset;

		if ( pSrc + pUnpack->m_nSize > static_cast<const char*>( pData ) + byteCount )
		{
			AssertMsg( 0, "CDmxElement::AddAttributesFromStructure_Internal: Read structure overrun!\n" );
			Warning( "Buffer underread! Mismatched type/type-descriptor.\n" );
			return;
		}
		...
		if ( IsArrayType( pUnpack->m_AttributeType ) )
		{
			AssertMsg( 0, "CDmxElement::AddAttributesFromStructure_Internal: Array attribute types not currently supported!\n" );
			continue;
		}

		if ( pUnpack->m_AttributeType == AT_VOID )
		{
			AssertMsg( 0, "CDmxElement::AddAttributesFromStructure_Internal: Binary blob attribute types not currently supported!\n" );
			continue;
		}

		if ( HasAttribute( pUnpack->m_pAttributeName ) )
		{
			AssertMsg( 0, "CDmxElement::AddAttributesFromStructure_Internal: Attribute %s already exists!\n", pUnpack->m_pAttributeName );
			continue;
		}

		{
			...
			else
			{
				...
				AssertMsg( nSize <= CDmxAttribute::AttributeDataSize( pUnpack->m_AttributeType ),
						   "CDmxElement::AddAttributesFromStructure_Internal: Incorrect size to unpack data into in attribute \"%s\"!\n", pUnpack->m_pAttributeName );
