...
//-----------------------------------------------------------------------------
class CUtlBuffer;
static constexpr const char* const g_pAttributeTypeName[AT_TYPE_COUNT] =
{
	"unknown", // AT_UNKNOWN
...
public:
	...
	int Push( const CUtlSymbol& symName );
	...
	void Reset( int stackLevel, const CUtlSymbol& symName );
...
CDmxKeyValues2ErrorStack::CDmxKeyValues2ErrorStack() :
	m_pFilename( "nullptr" ), m_nFileLine( 1 ), m_errorIndex( 0 ), m_maxErrorIndex( 0 )
{
}
...
// Not save symbols instead of pointers because the pointers can move!
//-----------------------------------------------------------------------------
int CDmxKeyValues2ErrorStack::Push( const CUtlSymbol& symName )
{
...
// Allows you to keep the same stack level, but change the name as you parse peers
//-----------------------------------------------------------------------------
void CDmxKeyValues2ErrorStack::Reset( int stackLevel, const CUtlSymbol& symName )
{
...
public:
	...
	CKeyValues2ErrorContext( const CUtlSymbol& symName )
	{
		Init( symName );
	}
	...
	void Reset( const CUtlSymbol& symName )
	{
		g_KeyValues2ErrorStack.Reset( m_stackLevel, symName );
	}

private:
	void Init( const CUtlSymbol& symName )
	{
...
static constexpr DmxElementDictHandle_t ELEMENT_DICT_HANDLE_INVALID = ~0;
...
public:
	...
	DmxElementDictHandle_t FindElement( CDmxElement* pElement ) const;
	DmxElementDictHandle_t FindElement( const DmObjectId_t& objectId ) const;
...
DmxElementDictHandle_t CDmxElementDictionary::InsertElement( CDmxElement* pElement )
{
	...
	const DmxElementDictHandle_t h = m_Dict.AddToTail();
...
{
	const int i = m_Attributes.AddToTail();
...
{
	const int i = m_ArrayAttributes.AddToTail();
...
// Finds an element into the table
//-----------------------------------------------------------------------------
DmxElementDictHandle_t CDmxElementDictionary::FindElement( CDmxElement* pElement ) const
{
	const int find = m_Dict.FindMatch( [&pElement]( const DictInfo_t& info ) { return info.m_pElement == pElement; } );
	if ( m_Dict.IsValidIndex( find ) )
		return find;
...
// Finds an element into the table
//-----------------------------------------------------------------------------
DmxElementDictHandle_t CDmxElementDictionary::FindElement( const DmObjectId_t& objectId ) const
{
	const int find = m_Dict.FindMatch( [&objectId]( const DictInfo_t & info ) { return IsUniqueIdEqual( objectId, info.m_Id ); } );
	if ( m_Dict.IsValidIndex( find ) )
		return find;
	return ELEMENT_DICT_HANDLE_INVALID;
}
...
{
	const int n = m_Attributes.Count();
	for ( int i = 0; i < n; ++i )
	{
		Assert( m_Attributes[i].m_nType == AT_OBJECTID );

		const DmxElementDictHandle_t hElement = FindElement( m_Attributes[i].m_ObjectId );
...
{
	const int n = m_ArrayAttributes.Count();
	for ( int i = 0; i < n; ++i )
	{
		...
		else
		{
			...
			const DmxElementDictHandle_t hElement = FindElement( m_ArrayAttributes[i].m_ObjectId );
			CDmxElement* pElement = GetElement( hElement );
			array.AddToTail( pElement );
		}
	}
...
void CDmxSerializerKeyValues2::SerializeElementAttribute( CUtlBuffer& buf, CDmxSerializationDictionary& dict, CDmxAttribute* pAttribute )
{
	...
	{
		buf.Printf( R"("%s" ")", g_pAttributeTypeName[AT_ELEMENT] );
		if ( pElement )
...
{
	const int nCount = pAttribute->GetArrayCount();
...
//-----------------------------------------------------------------------------
static int SortAttributeByName( CDmxAttribute* const* p1, CDmxAttribute* const* p2 )
{
	const char* pAttName1 = ( *p1 )->GetName();
	const char* pAttName2 = ( *p2 )->GetName();
...
{
	const int nCount = pElement->AttributeCount();
	CUtlVector<CDmxAttribute*> ppAttributes( 0, nCount );
	for ( int i = 0; i < nCount; ++i )
		ppAttributes.AddToTail( pElement->GetAttribute( i ) );

	// Sort by name
	ppAttributes.Sort( SortAttributeByName );
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		const DmAttributeType_t nAttrType = pAttribute->GetType();
		if ( nAttrType != AT_ELEMENT )
			buf.Printf( R"("%s" "%s" )", pName, g_pAttributeTypeName[nAttrType] );
		else
...
bool CDmxSerializerKeyValues2::SaveElement( CUtlBuffer& buf, CDmxSerializationDictionary& dict, CDmxElement* pElement, bool bWriteDelimiters )
{
	...
	buf.Printf( R"("id" "%s" )", g_pAttributeTypeName[AT_OBJECTID] );
	buf.PutChar( '\"' );
	::Serialize( buf, pElement->GetId() );
	buf.PutString( "\"\n" );
...
bool CDmxSerializerKeyValues2::Serialize( CUtlBuffer& outBuf, CDmxElement* pRoot, const char* pFormatName )
{
	...
	const bool bFlatMode = !Q_stricmp( pFormatName, "keyvalues2_flat" );

	// Save elements, attribute links
	CDmxSerializationDictionary dict;
	dict.BuildElementList( pRoot, bFlatMode );

	for ( DmxSerializationHandle_t i = dict.FirstRootElement(); i != ELEMENT_DICT_HANDLE_INVALID; i = dict.NextRootElement(i) )
	{
...
void CDmxSerializerKeyValues2::EatWhitespacesAndComments( CUtlBuffer& buf )
{
	...
	const int nMaxPut = buf.TellMaxPut() - buf.TellGet();
	int nOffset = 0;
	while ( nOffset < nMaxPut )
	{
		// Eat whitespaces, keep track of line count
		const char* pPeek;
		while ( ( pPeek = static_cast<const char*>( buf.PeekGet( sizeof( char ), nOffset ) ) ) != nullptr )
		{
			if ( !isspace( *pPeek ) )
				break;
		...
		pPeek = static_cast<const char*>( buf.PeekGet( 2 * sizeof( char ), nOffset ) );
...
CDmxSerializerKeyValues2::TokenType_t CDmxSerializerKeyValues2::ReadToken( CUtlBuffer& buf, CUtlBuffer& token )
{
	...
	const char c = *static_cast<const char*>( buf.PeekGet() );
	switch( c )
	{
	case '{':
		nLength = 1;
	...
	case '\"':
		...
		if ( ( nLength <= 1 ) || ( *static_cast<const char*>( buf.PeekGet( nLength - 1 ) ) != '\"' ) )
		{
	...
	token.SetExternalBuffer( const_cast<void*>( buf.PeekGet() ), nLength, nLength, CUtlBuffer::TEXT_BUFFER | CUtlBuffer::READ_ONLY );
	buf.SeekGet( CUtlBuffer::SEEK_CURRENT, nLength );

	// Count the number of crs in the token + update the current line
	const char* pMem = token.String();
...
bool CDmxSerializerKeyValues2::UnserializeElementAttribute( CUtlBuffer& buf, DmxElementDictHandle_t hElement, const char* pAttributeName, const char* pElementType )
{
	...
	const bool bOk = UnserializeElement( buf, pElementType, &h );
	if ( bOk )
	{
...
bool CDmxSerializerKeyValues2::UnserializeElementArrayAttribute( CUtlBuffer& buf, DmxElementDictHandle_t hElement, const char* pAttributeName )
{
	...
	while ( buf.IsValid() )
	{
		...
		char* pElementType = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
		...
		if ( !Q_strncmp( pElementType, g_pAttributeTypeName[AT_ELEMENT], nLength ) )
		{
			...
			char* pElementId = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
		...
		else
		{
			...
			const bool bOk = UnserializeElement( buf, pElementType, &hArrayElement );
			if ( !bOk )
				return false;
...
bool CDmxSerializerKeyValues2::UnserializeAttributeValueFromToken( CDmxAttribute* pAttribute, DmAttributeType_t type, CUtlBuffer& tokenBuf )
{
	...
	const char* pBuf = tokenBuf.String();
	int nLength = tokenBuf.TellMaxPut();
	char* pTemp = static_cast<char*>( stackalloc( nLength + 1 ) );

	const bool bIsString = ( type == AT_STRING ) || ( type == AT_STRING_ARRAY );
	if ( !bIsString )
	{
...
// Reads an attribute for an element
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeAttribute( CUtlBuffer& buf, DmxElementDictHandle_t hElement, const char* pAttributeName, DmAttributeType_t nAttrType )
{
	// Read the attribute value
	CUtlBuffer tokenBuf;
	const TokenType_t token = ReadToken( buf, tokenBuf );
	if ( token != TOKEN_DELIMITED_STRING )
	{
	...
	{
		CUtlCharConversion* pConv = GetCStringCharConversion();
		const int nLength = tokenBuf.PeekDelimitedStringLength( pConv );
		char* pElementId = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
		tokenBuf.GetDelimitedString( pConv, pElementId, nLength );
	...
	case AT_ELEMENT:
		{
			...
			CUtlCharConversion* pConv = GetCStringCharConversion();
			const int nLength = tokenBuf.PeekDelimitedStringLength( pConv );
			char* pAttributeValue = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
			tokenBuf.GetDelimitedString( pConv, pAttributeValue, nLength );

			// No string? that's ok, it means we have a nullptr pointer
...
bool CDmxSerializerKeyValues2::UnserializeElement( CUtlBuffer& buf, const char* pElementType, DmxElementDictHandle_t* pHandle )
{
	*pHandle = ELEMENT_DICT_HANDLE_INVALID;
	...
	// Create the element
	const DmxElementDictHandle_t hElement = CreateDmxElement( pElementType );
	...
	while ( buf.IsValid() )
	{
		...
		CUtlCharConversion* pConv = GetCStringCharConversion();
		int nLength = tokenBuf.PeekDelimitedStringLength( pConv );
		char* pAttributeName = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
		...
		pConv = GetCStringCharConversion();
		nLength = tokenBuf.PeekDelimitedStringLength( pConv );
		char* pAttributeType = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
		...
		for ( int i = 0; i < AT_TYPE_COUNT; ++i )
		{
			...
			{
				nAttrType = static_cast<DmAttributeType_t>( i );
				break;
			}
		}
...
bool CDmxSerializerKeyValues2::UnserializeElement( CUtlBuffer& buf, DmxElementDictHandle_t* pHandle )
{
	*pHandle = ELEMENT_DICT_HANDLE_INVALID;
	...
	CUtlCharConversion* pConv = GetCStringCharConversion();
	const int nLength = tokenBuf.PeekDelimitedStringLength( pConv );
	char* pTypeName = static_cast<char*>( stackalloc( nLength * sizeof( char ) ) );
